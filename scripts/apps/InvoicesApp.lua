...
function RoleplayPhone:drawInvoiceDetail()
    ...
    local function drawDetailMultiline(label, value)
        local lineH    = 0.016
        local textSize = 0.012
        local textPad  = 0.004
        local maxWidth = fieldW - textPad * 2
        ...
        for _, word in ipairs(words) do
            ...
            for ci, chunk in ipairs(wordChunks) do
                if ci > 1 then
                    table.insert(lines, cur); cur = chunk
                elseif cur == "" then
                    cur = chunk
                elseif getTextWidth(textSize, cur .. " " .. chunk) <= maxWidth then
                    cur = cur .. " " .. chunk
                else
                    table.insert(lines, cur); cur = chunk
                end
        ...
        local boxH = math.max(0.036, #lines * lineH + 0.024)
        curY = curY - boxH
        self:drawRect(fieldX, curY, fieldW, boxH, 0.10, 0.13, 0.19, 1.0)
        setTextAlignment(RenderText.ALIGN_LEFT)
        setTextBold(false)
        setTextColor(0.55, 0.65, 0.80, 0.85)
        renderText(fieldX + textPad, curY + boxH - 0.014, 0.009, label)
        setTextColor(1, 1, 1, 1)
        for i, line in ipairs(lines) do
            local lineY = curY + boxH - 0.014 - (i * lineH)
            renderText(fieldX + textPad, lineY, 0.013, line)
        end
    end
    ...
    if inv.notes and inv.notes ~= "" then
        drawDetailMultiline(g_i18n:getText("invoices_detail_notes"), inv.notes)
    end
...
function RoleplayPhone:drawCreateInvoice()
    ...
    local headerY = py + ph - 0.055 - headerH
    self:drawRect(px, headerY, pw, headerH, 0.10, 0.13, 0.20, 1.0)
    local btnW = 0.055 * self.arScale
    local btnH = 0.030
    self:drawButton("btn_back", px+0.006, headerY+0.010, btnW, btnH,
                    g_i18n:getText("ui_btn_back"), 0.18, 0.20, 0.28, 0.011)
    -- Send button — top right, mirrors Back button, green
    self:drawButton("btn_send_invoice", px+pw-btnW-0.006, headerY+0.010, btnW, btnH,
                    g_i18n:getText("invoices_btn_send_short"), 0.10, 0.38, 0.18, 0.011)
    setTextAlignment(RenderText.ALIGN_CENTER)
    setTextBold(true)
    setTextColor(1, 1, 1, 1)
    ...
    -- ── Notes field (multiline word wrap) ──
    local notesH = fldH * 2.0
    curY = curY - notesH - fldGap
    self:drawField("field_notes", col1X, curY, colW, notesH,
                   g_i18n:getText("invoices_field_notes_label"), self.form.notes,
                   self.form.activeField == "notes", true)
