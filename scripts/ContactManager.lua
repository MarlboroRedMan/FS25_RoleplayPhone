...
function ContactManager:saveToXML(xmlFile, key)
    ...
    -- Per-user contacts: all client players stored under users(N)
    local userList = {}
    for userId, contacts in pairs(self.userContacts) do
        table.insert(userList, { userId = userId, contacts = contacts })
    end
    ...
    for ui, entry in ipairs(userList) do
        ...
        setXMLInt(xmlFile, userKey .. "#userId", entry.userId)
...
function ContactManager:loadFromXML(xmlFile, key)
    ...
    for ui = 0, userCount - 1 do
        ...
        if userId then
            ...
            self.userContacts[userId] = contacts
        end
    end
