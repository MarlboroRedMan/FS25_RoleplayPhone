...
function InvoiceSave:loadFromXML(xmlFile, key)
    ...
    while true do
        ...
        -- Track highest ID seen so we can guarantee nextInvoiceId stays above it
        if inv.id and inv.id > maxId then maxId = inv.id end
        i = i + 1
    end

    -- Safety: ensure counter is always above every existing ID (handles old time-based saves)
